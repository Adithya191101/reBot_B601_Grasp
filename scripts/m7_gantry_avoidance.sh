#!/usr/bin/env bash
# M7 gate: static gantry collision avoidance (visible re-routing).
#
# The M6 orchestration flow (scripts/m6_cumotion_trials.sh) REUSED
# UNCHANGED -- same images, same M5 sim bridge, same DDS profile, same
# stack bring-up and teardown -- with the M7 sampler/runner/verifier and
# artifacts/m7:
#
...
#               standalone action server)
#   sampler:    scripts/m7_sample_pairs.py        (host: gantry-blocked pairs)
#   runner:     scripts/m7_trial_runner.py        (inside container2)
#   verify:     scripts/m7_verify_trials.py       (host, mesh recheck+metrics)
#
...
#
# Writes artifacts/m7/gantry_avoidance.json (verdict + peak VRAM).
# Exit 0 iff the gate passed.
set -uo pipefail
...
STACK_NAME="rebot_m7_stack"
CU_NAME="rebot_m7_cumotion_stack"
ART="$REPO/artifacts/m7"
BRIDGE_TIMEOUT="${BRIDGE_TIMEOUT:-300}"
BRIDGE_DURATION="${BRIDGE_DURATION:-3600}"

mkdir -p "$ART"
rm -f "$ART/bridge_ready.json" "$ART/trials_raw.json" \
      "$ART/gantry_avoidance.json"
...
echo "== [1/8] gantry-blocked pose pairs (host, deterministic) =="
"$ISAAC_PY" "$REPO/scripts/m7_sample_pairs.py" || exit 1
...
echo "== [6/8] running the gantry-avoidance pair trials =="
docker exec -e ROS_DOMAIN_ID=42 \
    -e FASTRTPS_DEFAULT_PROFILES_FILE=/work/config/fastdds_udp.xml \
    "$CU_NAME" bash -c '
    source /opt/ros/jazzy/setup.bash
    python3 /work/scripts/m7_trial_runner.py
...
"$ISAAC_PY" "$REPO/scripts/m7_verify_trials.py"
VERIFY_RC=$?

if [ "$VERIFY_RC" -eq 0 ]; then
    echo "M7 GANTRY-AVOIDANCE GATE: PASS"
else
    echo "M7 GANTRY-AVOIDANCE GATE: FAIL (runner rc=$RUNNER_RC; see" \
